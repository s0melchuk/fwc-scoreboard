...
/**
 * An immutable snapshot of a match in progress: its identity, the two teams involved, the current
 * score, and the order in which it was started relative to other matches.
 *
 * <p>
 * {@code startOrder} is a monotonically increasing sequence number assigned when the match is
 * started, not a wall-clock timestamp. Using a logical clock rather than {@link java.time.Instant}
 * avoids flaky tie-breaking when two matches start within the same clock tick, and keeps ordering
